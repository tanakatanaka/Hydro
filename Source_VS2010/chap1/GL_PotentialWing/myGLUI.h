...
#define NUM_MAX  220//最大分割数（最大NUM_MAX*NUM_MAXの格子数）
...
float xi0 =   -0.1;//半径a(radCylinder)で割った値
float eta0 =  0.2;//半径a(radCylinder)で割った値
float Gamma = 0.0;//渦の強さ（循環）
...
void control_cb(int control)
{
	...
	else if(control == STOP_ID) flagExecute = 0;
...
void setupGLUI()
{
	...
	glui->add_statictext_to_panel(kindPanel, "radCylinder(Wing)" );
	...
	spinner_Radius -> set_speed( 1.0 );
	GLUI_Spinner *spinner_Xi0;
	spinner_Xi0 = new GLUI_Spinner( kindPanel, "xi0/R", &xi0);
	spinner_Xi0 -> set_float_limits(-1.0, 1.0 );
	spinner_Xi0 -> set_speed( 1.0 );
	GLUI_Spinner *spinner_Eta0;
	spinner_Eta0 = new GLUI_Spinner( kindPanel, "eta0/R", &eta0);
	spinner_Eta0 -> set_float_limits(-1.0, 1.0 );
	spinner_Eta0 -> set_speed( 1.0 );
	...
	glui->add_separator_to_panel(kindPanel);
	new GLUI_Checkbox(kindPanel, "Voltex", &flagVoltex );
	...
	glui->add_separator_to_panel(dispPanel);
	new GLUI_Checkbox(dispPanel, "pressure", &flagPressure );
	...
	glui->add_separator_to_panel(dispPanel);
	new GLUI_Checkbox(dispPanel, "cylinder(wing)", &flagCylinder );

	glui->add_separator_to_panel(dispPanel);
	new GLUI_Checkbox(dispPanel, "coordinates", &flagCoord );
