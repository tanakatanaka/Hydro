...
int alpha_int = 0;
float radCylinder = 0.1;//円柱の半径（ダブレット）
...
GLUI_EditText *velocity_edit;
GLUI_EditText *alpha_edit;
GLUI_EditText *Radius_edit;
GLUI_EditText *Gamma_edit;
...
void setupGLUI()
{
	...
	new GLUI_Checkbox(kindPanel, "Doublet", &flagDoublet );
	...
	spinner_Gamma -> set_float_limits(-5.0, 5.0 );
	...
	spinner_nLine -> set_int_limits(1, 50 );
	...
	glui->add_separator_to_panel(dispPanel);
//	new GLUI_Checkbox(dispPanel, "cylinder", &flagCylinder );
	glui->add_statictext_to_panel(dispPanel, "Cylinder" );
	radioC = new GLUI_RadioGroup(dispPanel, &cylinder);
	new GLUI_RadioButton(radioC, "translucent");
	new GLUI_RadioButton(radioC, "opaque");
