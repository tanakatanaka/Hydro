...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GP_DuctFS1_2");
	//ウィンドウのサイズ変更
...
void initData()
{
	...
	int NY1_2 = floor((yy0 + rect.obs_width/2.0) / DY + eps);//障害物上端位置
	int NY1_1 = floor((yy0 - rect.obs_width/2.0) / DY + eps);//障害物下端位置
	...
	int NX2_1 = floor((rect.obs_left - rect.obs_thick/2.0) / DX + eps);//障害物左端位置
	int NX2_2 = floor((rect.obs_left + rect.obs_thick/2.0) / DX + eps);//障害物右端位置
	...
	for(j = 0; j < texHeight; j++)
		for(i = 0; i < texWidth; i++)
		{
			...
			//if(g_type[i][j] >= OBS_LEFT) velX[4 * k + 0] = 0.0;//障害物表面
			//速度Y
			velY[4 * k + 0] = 0.0;
			velY[4 * k + 1] = 0.0;//x微分
			velY[4 * k + 2] = 0.0;//y微分
	...
	rigid[0].kind = CUBE;
	rigid[0].color = GRAY;
	rigid[0].vPos = Vector3D(-rect.size.x/2.0 + rect.obs_left, -rect.obs_dist/2.0, 0.0);
	rigid[0].vSize = Vector3D(rect.obs_thick, rect.obs_width, 1.0);
	//上のの障害物（直方体）
	rigid[1].kind = CUBE;
	rigid[1].color = GRAY;
	rigid[1].vPos = Vector3D(-rect.size.x/2.0 + rect.obs_left, rect.obs_dist/2.0, 0.0);
