...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GP_DuctFS1_1");
	//ウィンドウのサイズ変更
...
void initData()
{
	...
	//障害物境界の分割点番号
	float eps = 0.5;
	int NX1 = floor((rect.obs_left - rect.obs_thick / 2.0) / DX + eps);//障害物左端位置
	int NX2 = floor((rect.obs_left + rect.obs_thick / 2.0) / DX + eps);//障害物右端位置
	int NY2 = floor((rect.obs_posY + rect.obs_width/2.0) / DY + eps);//障害物上端位置
	int NY1 = floor((rect.obs_posY - rect.obs_width/2.0) / DY + eps);//障害物下端位置

	int NY_ObsW = floor(rect.obs_width / DY + eps);
	//格子間隔の整数倍か
	float left1, left2, bottom1, bottom2;
	left1 = DX * (float)NX1;//左端から障害物左端まで
		...
		for(int m = 0; m < dispSpeed; m++)//無次元タイムステップdeltaTは変えずに表示速度だけ変える
		{
			...
			for (i = 0; i <= NX; i++)
				for (j = 0; j <= NY; j++) 
				{
					...
					else if(g_type[i][j] == OUTLET) velX[k] = 0.0;//prs[i-1][j];//Dirichlet（流出口）
					else if(g_type[i][j] == TOP) velX[k] = velX[4 * (i + (NY-1) * texWidth) + 3];//prs[i][NY-1];
					else if(g_type[i][j] == BOTTOM) velX[k] = velX[4 * (i + texWidth) + 3];//prs[i][1];
					else if(g_type[i][j] == OBS_LEFT) velX[k] = velX[4 * (i-1 + j * texWidth) + 3];//prs[i-1][j];
					else if(g_type[i][j] == OBS_RIGHT) velX[k] = velX[4 * (i+1 + j * texWidth) + 3];//prs[i+1][j];
					else if(g_type[i][j] == OBS_TOP) velX[k] = velX[4 * (i + (j+1) * texWidth) + 3];//prs[i][j+1];
					else if(g_type[i][j] == OBS_BOTTOM) velX[k] = velX[4 * (i + (j-1) * texWidth) + 3];//prs[i][j-1];
					else if(g_type[i][j] == OBS_UL) velX[k] = velX[4 * (i-1 + (j+1) * texWidth) + 3];//prs[i-1][j+1];
					else if(g_type[i][j] == OBS_UR) velX[k] = velX[4 * (i+1 + (j+1) * texWidth) + 3];//prs[i+1][j+1];
					else if(g_type[i][j] == OBS_LL) velX[k] = velX[4 * (i-1 + (j-1) * texWidth) + 3];//prs[i-1][j-1];
					else if(g_type[i][j] == OBS_LR) velX[k] = velX[4 * (i+1 + (j-1) * texWidth) + 3];//prs[i+1][j-1];
				}
...
//printf("maxOmg=%f minOmg=%f \n", maxVel0, minVel0);
	...
	if(flagCoordinates) drawWorldCoordinates(5.0);
	//バッファの切り替え
	glutSwapBuffers();	
}
