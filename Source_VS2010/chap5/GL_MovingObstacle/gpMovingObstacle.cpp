...
enum Type{INSIDE, LEFT, RIGHT, TOP, BOTTOM, OBS_LEFT, OBS_TOP, OBS_BOTTOM, OBS_RIGHT, OBSTACLE, OBS_UL, OBS_UR, OBS_LL, OBS_LR};
Type type[NUM_MAX_X][NUM_MAX_Y]; //格子点のタイプ
int NX, NY, NX1, NY1, NX2, NY2;
float DX, DY, A1, A2, A3;
Vector2D obsPos = Vector2D();
Vector2D obsVel = Vector2D();

Particle2D p[MAX_PARTICLE];
bool flagResetting = false;
int count;
float direction = 1.0;//障害物移動方向
...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GL_MovingObstacle");
	//ウィンドウのサイズ変更
...
void init(void)
{
	...
	NY2 = floor((rect.obs_posY + rect.obs_width/2.0) / DY + eps);//障害物上端位置
	NY1 = floor((rect.obs_posY - rect.obs_width/2.0) / DY + eps);//障害物下端位置
	...
	for(i = 0; i <= NX; i++)
		for(j = 0; j <= NY; j++)
		{
			...
			if(i == 0) type[i][j] = LEFT;//左端
			if(i == NX) type[i][j] = RIGHT;//右端
	...
	obsPos = Vector2D(rect.obs_left, rect.obs_posY);//ダクト左下を原点とした位置
	obsVel = Vector2D(direction * obsSpeed, 0.0);//移動速度
	...
	for(j = 1; j < NY; j++)
		for (i = 1; i < NX; i++)
		{
			p[countP].pos = Vector2D((float)i * DX, (float)j * DY);
			p[countP].size = sizeParticle;
			p[countP].speedC = 1.0;
			if(i <= NX/3) p[countP].color = RED;
			else if(i <= 2 * NX/3)  p[countP].color = GREEN;			
...
//printf("count=%d \n", count);		
	...
	if( flagOmega ) drawContour(Omg, minOmg, maxOmg, BLACK);

	if( flagVelocity ) drawVelocity();
	if( flagGrid ) drawGrid();

	drawParticle(deltaT);//粒子アニメーション
	...
	{
		Courant = obsSpeed * deltaT / rect.delta.x;	
		diff_num = (1.0 / Re) * deltaT / (rect.delta.x * rect.delta.x);//拡散数
	}
	else
	{
		Courant = obsSpeed * deltaT / rect.delta.y;	
...
void calculate(float deltaT)
{
	...
	//Poissonの方程式を解く(1回だけに簡略化）
	for (i = 0; i <= NX; i++)
		for (j = 0; j <= NY; j++) 
		{
			if(type[i][j] == INSIDE) continue;
			else if(type[i][j] == LEFT)  Prs[i][j] = Prs[i+1][j];//Neumann
			else if(type[i][j] == RIGHT) Prs[i][j] = 0.0;//Dirichlet
...
void drawObstacle()
{
	...
	glBegin(GL_QUADS);
	glVertex2f(rect.left0.x + scale.x * (obsPos.x - rect.obs_thick/2.0), rect.left0.y + scale.y * (obsPos.y - rect.obs_width/2.0));
	glVertex2f(rect.left0.x + scale.x * (obsPos.x + rect.obs_thick/2.0), rect.left0.y + scale.y * (obsPos.y - rect.obs_width/2.0));
	glVertex2f(rect.left0.x + scale.x * (obsPos.x + rect.obs_thick/2.0), rect.left0.y + scale.y * (obsPos.y + rect.obs_width/2.0));
	glVertex2f(rect.left0.x + scale.x * (obsPos.x - rect.obs_thick/2.0), rect.left0.y + scale.y * (obsPos.y + rect.obs_width/2.0));
	glEnd();
...
void calcObsPos()
{
	if(flagObsStop) { obsVel = Vector2D(); return; }
	if(obsPos.x >= 1.7) direction = -1.0;
	if(obsPos.x <= 0.3) direction =  1.0;
	obsVel.x = direction * obsSpeed;
	obsPos.x += obsVel.x * deltaT;

	float eps = 0.5;
	NX1 = floor((obsPos.x - rect.obs_thick / 2.0) / DX + eps);//障害物左端位置
	NX2 = floor((obsPos.x + rect.obs_thick / 2.0) / DX + eps);//障害物右端位置
	NY2 = floor((obsPos.y + rect.obs_width/2.0) / DY + eps);//障害物上端位置
	NY1 = floor((obsPos.y - rect.obs_width/2.0) / DY + eps);//障害物下端位置
