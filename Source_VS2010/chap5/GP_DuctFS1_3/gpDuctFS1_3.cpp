...
void initData()
{
	...
	float yy0 = rect.size.y / 2.0;//左側障害物の中心
	obsTop = yy0 + rect.obs_width / 2.0;
	obsBottom = yy0 - rect.obs_width / 2.0;
	
	//左側の障害物境界の分割点番号
	float eps = 0.5;
	int NX1_1 = floor((rect.obs_left - rect.obs_thick/2.0) / DX + eps);//障害物左端位置
	int NX1_2 = floor((rect.obs_left + rect.obs_thick/2.0) / DX + eps);//障害物右端位置
	int NY1_2 = floor(obsTop / DY + eps);//障害物上端位置
	int NY1_1 = floor(obsBottom / DY + eps);//障害物下端位置
...
*/
	//下の障害物境界の分割点番号
	yy0 = (rect.size.y - rect.obs_dist2) / 2.0;//下の障害物の中心
	int NX2_1 = floor((rect.obs_left + rect.obs_dist1 - rect.obs_thick/2.0) / DX + eps);//障害物左端位置
	int NX2_2 = floor((rect.obs_left + rect.obs_dist1 + rect.obs_thick/2.0) / DX + eps);//障害物右端位置
	int NY2_2 = floor((yy0 + rect.obs_width/2.0) / DY + eps);//障害物上端位置
	int NY2_1 = floor((yy0 - rect.obs_width/2.0) / DY + eps);//障害物下端位置
	//上の障害物境界の分割点番号
	yy0 = (rect.size.y + rect.obs_dist2) / 2.0;
	int NX3_1 = floor((rect.obs_left + rect.obs_dist1 - rect.obs_thick/2.0) / DX + eps);//障害物左端位置
	int NX3_2 = floor((rect.obs_left + rect.obs_dist1 + rect.obs_thick/2.0) / DX + eps);//障害物右端位置
	int NY3_2 = floor((yy0 + rect.obs_width/2.0) / DY + eps);//障害物上端位置
	int NY3_1 = floor((yy0 - rect.obs_width/2.0) / DY + eps);//障害物下端位置
	...
	for(i = 0; i <= NX; i++)
		for(j = 0; j <= NY; j++)
		{
			...
			//左側の障害物
			if(i == NX1_1 && j > NY1_1 && j < NY1_2) g_type[i][j] = OBS_LEFT;//障害物左端
			if(i == NX1_2 && j > NY1_1 && j < NY1_2) g_type[i][j] = OBS_RIGHT;//障害物右端
			if(i >= NX1_1 && i <= NX1_2 && j == NY1_2) g_type[i][j] = OBS_TOP;//障害物上端
			if(i >= NX1_1 && i <= NX1_2 && j == NY1_1) g_type[i][j] = OBS_BOTTOM;//障害物下端
			if(i > NX1_1 && i < NX1_2 && j > NY1_1 && j < NY1_2) g_type[i][j] = OBSTACLE;//障害物内部
			if(i == NX1_1 && j == NY1_1) g_type[i][j] = OBS_LL;//左下
			if(i == NX1_1 && j == NY1_2) g_type[i][j] = OBS_UL;//左上
			if(i == NX1_2 && j == NY1_1) g_type[i][j] = OBS_LR;//右下
			if(i == NX1_2 && j == NY1_2)  g_type[i][j] = OBS_UR;//右上
			//下の障害物
			if(i == NX2_1 && j > NY2_1 && j < NY2_2) g_type[i][j] = OBS_LEFT;//障害物左端
			if(i == NX2_2 && j > NY2_1 && j < NY2_2) g_type[i][j] = OBS_RIGHT;//障害物右端
			if(i >= NX2_1 && i <= NX2_2 && j == NY2_2) g_type[i][j] = OBS_TOP;//障害物上端
			if(i >= NX2_1 && i <= NX2_2 && j == NY2_1) g_type[i][j] = OBS_BOTTOM;//障害物下端
			if(i > NX2_1 && i < NX2_2 && j > NY2_1 && j < NY2_2) g_type[i][j] = OBSTACLE;//障害物内部
			if(i == NX2_1 && j == NY2_1) g_type[i][j] = OBS_LL;//左下
			if(i == NX2_1 && j == NY2_2) g_type[i][j] = OBS_UL;//左上
			if(i == NX2_2 && j == NY2_1) g_type[i][j] = OBS_LR;//右下
			if(i == NX2_2 && j == NY2_2)  g_type[i][j] = OBS_UR;//右上
			//上の障害物
			if(i == NX3_1 && j > NY2_1 && j < NY2_2) g_type[i][j] = OBS_LEFT;//障害物左端
			if(i == NX3_2 && j > NY2_1 && j < NY2_2) g_type[i][j] = OBS_RIGHT;//障害物右端
			if(i >= NX3_1 && i <= NX2_2 && j == NY2_2) g_type[i][j] = OBS_TOP;//障害物上端
			if(i >= NX3_1 && i <= NX2_2 && j == NY2_1) g_type[i][j] = OBS_BOTTOM;//障害物下端
			if(i > NX3_1 && i < NX3_2 && j > NY3_1 && j < NY3_2) g_type[i][j] = OBSTACLE;//障害物内部
			if(i == NX3_1 && j == NY3_1) g_type[i][j] = OBS_LL;//左下
			if(i == NX3_1 && j == NY3_2) g_type[i][j] = OBS_UL;//左上
			if(i == NX3_2 && j == NY3_1) g_type[i][j] = OBS_LR;//右下
			if(i == NX3_2 && j == NY3_2)  g_type[i][j] = OBS_UR;//右上
		}
...
/*
			for (i = 1; i < NX; i++)
				...
				{
					k = 4 * (i + j * texWidth);
					if(g_type[i][j] >= OBS_LEFT) //障害物表面
					{
						velX[k] = 0.0;
						velY[k] = 0.0;
					}
					if(g_type[i][j] == BOTTOM || g_type[i][j] == TOP){//壁面
						velX[k] = (float)flagIdeal;
						velY[k] = 0.0;
					}
				}*/
			...
			for (i = 0; i <= NX; i++)
				for (j = 0; j <= NY; j++) 
				{
					...
					else if(g_type[i][j] == INLET)  velX[k] = velX[4*(i+1 + j * texWidth) + 3];//Neumann(流入口）
					else if(g_type[i][j] == OUTLET) velX[k] = 0.0;//Dirichlet（流出口）
