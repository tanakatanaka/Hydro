...
struct Rect
{
	...
	float obs_dist1;//左側(1個)の障害物と右側(2個）の障害物の間隔
	float obs_dist2;//2個の障害物中心間の距離（中心軸を挟んで対称の位置）
	int meshX;     //分割数(X)
	int meshY;     //分割数(Y)
};
Rect rect = {
	Vector2D(20.0, 10.0),//size
	2.0, 0.4, 0.4, 2.0, 2.0,  //obs_left, thick, width, dist
	200,//meshX
	100 //meshY
};
...
GLUI_EditText *dist1_edit;
GLUI_EditText *dist2_edit;
...
void setupGLUI()
{
	...
	dist1_edit = new GLUI_EditText(paraPanel,"obs_dist1", &rect.obs_dist1);
	dist2_edit = new GLUI_EditText(paraPanel,"obs_dist2", &rect.obs_dist2);
