...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GL_DuctFS1_1");
	//ウィンドウのサイズ変更
	...
	while (cnt < iteration)
	{
		...
		for (i = 0; i <= NX; i++)
			for (j = 0; j <= NY; j++) 
			{
				...
				else if(type[i][j] == INLET)  Prs[i][j] = Prs[i+1][j];//Neumann(流入口）
				else if(type[i][j] == OUTLET) Prs[i][j] = 0.0;//Dirichlet//Prs[i-1][j];（流出口）
