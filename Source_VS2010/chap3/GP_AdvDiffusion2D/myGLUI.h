...
int flagCoordShow = 0;
int flagFloor = 0;
int flagRotation = 0;//回転速度場

Vector2D centerInit = Vector2D(2.5, 5.0) ;//これまでの10倍
Vector2D velocity = Vector2D(1.0, 0.0);//直線走行のときの速度
float speedR = 0.5;//回転速度場のときの速度係数
float diff_coef = 0.0;//拡散係数
float radiusInit =1.0;// 0.1;//初期中心温度（濃度）半径
float adjustH = 3.0;//0.5;//表示上の高さ調整
int boundary = 0;
int color = 0;
int thinningN = 1;
int profile = 0;//円柱
float deltaT0 = 0.01;//設定値
...
GLUI_EditText *centerX_edit;
GLUI_EditText *centerY_edit;
GLUI_EditText *velX_edit;
GLUI_EditText *velY_edit;
GLUI_EditText *deltaT_edit;
GLUI_EditText *speedR_edit;
GLUI_EditText *diff_edit;
GLUI_EditText *nMesh_edit;
GLUI_EditText *adjustH_edit;
...
void setupGLUI()
{
	...
	centerX_edit = new GLUI_EditText(paraPanel,"centerX", &centerInit.x);
	centerX_edit ->set_w(80);
	centerY_edit = new GLUI_EditText(paraPanel,"centerY", &centerInit.y);
	centerY_edit ->set_w(80);
	velX_edit = new GLUI_EditText(paraPanel,"velX", &velocity.x);
	velX_edit ->set_w(80);
	velY_edit = new GLUI_EditText(paraPanel,"velY", &velocity.y);
	velY_edit ->set_w(80);
	speedR_edit = new GLUI_EditText(paraPanel,"speedR", &speedR);
	speedR_edit ->set_w(80);
	diff_edit  = new GLUI_EditText(paraPanel, "diff_coef", &diff_coef);
	diff_edit ->set_w(80);
	deltaT_edit = new GLUI_EditText(paraPanel,"deltaT0", &deltaT0);
	...
	thinning_edit ->set_w(60);
 	...
 	new GLUI_Checkbox(glui, "Rotation", &flagRotation);
