...
float centerInit = 0.1;//その中心点

float speed = 0.1;//profileの移流速度
float diff_coef = 0.0001;//拡散係数
float deltaT0 = 0.01;    //時間刻み（設定値）
...
void setupGLUI()
{
	...
	speed_edit = new GLUI_EditText(paraPanel,"speed[m/s]", &speed);
	...
	radioM = new GLUI_RadioGroup(methodPanel, &method, 4);
	new GLUI_RadioButton(radioM, "upwind");
	new GLUI_RadioButton(radioM, "CIP");
	new GLUI_RadioButton(radioM, "semi-implicit");
	new GLUI_RadioButton(radioM, "fully-implicit");
