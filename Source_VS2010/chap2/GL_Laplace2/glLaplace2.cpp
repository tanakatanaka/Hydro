...
float Phi[NUM_MAX_X][NUM_MAX_Y];//流れ関数
...
enum Type{INSIDE, INLET, OUTLET, TOP, BOTTOM, OBS_LEFT, OBS_TOP, OBS_RIGHT, OBSTACLE, CORNER_UL, CORNER_UR};
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GL_Laplace2");//GLUIが対象とするウィンドウ
	//ウィンドウのサイズ変更
...
void init(void)
{
	...
	for(i = 0; i <= rect.nMeshX; i++)
		for(j = 0; j <= rect.nMeshY; j++) Phi[i][j] = 0.0;
