...
enum Type{INSIDE, INLET, OUTLET, TOP, BOTTOM, OBS_LEFT, OBS_TOP, OBS_RIGHT, OBSTACLE, OBS_UL, OBS_UR};
...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GL_DuctPsiOmega");
	//ウィンドウのサイズ変更
...
void init(void)
{
	...
	int nMeshY_ObsTop = floor(rect.obs_width / rect.delta.y + eps);
	//格子間隔の整数倍か
	float left1, left2, top;
	left1 = rect.delta.x * (float)nMeshX1;//左端から障害物左端まで
	left2 = rect.obs_left - rect.obs_thick/2.0;
	top = rect.delta.y * (float)nMeshY_ObsTop;
	eps = 0.00001;
	if(fabs(left1 - left2) > eps || fabs(rect.obs_width - top) > eps)
	{
	...
	for(i = 0; i <= NX; i++)
		for(j = 0; j <= NY; j++)
		{
			...
			if(i == nMeshX1 && j <nMeshY_ObsTop) type[i][j] = OBS_LEFT;//障害物左端
			if(i >= nMeshX1 && i <= nMeshX2 && j ==nMeshY_ObsTop) type[i][j] = OBS_TOP;//障害物上端
			if(i == nMeshX2 && j <nMeshY_ObsTop) type[i][j] = OBS_RIGHT;//障害物右端
			if(i > nMeshX1 && i < nMeshX2 && j <nMeshY_ObsTop) type[i][j] = OBSTACLE;//障害物内部
			if(i == nMeshX1 && j == 0) type[i][j] = OBS_UL;
			if(i == nMeshX1 && j ==nMeshY_ObsTop)  type[i][j] = OBS_UL;
			if(i == nMeshX2 && j == 0) type[i][j] = OBS_UR;
			if(i == nMeshX2 && j ==nMeshY_ObsTop)  type[i][j] = OBS_UR;
		}
	...
	for(j = 0; j <= rect.nMeshY; j++)
		...
		{
			if(type[i][j] == BOTTOM || type[i][j] >= OBS_LEFT) Psi[i][j] = 0.0;
...
void display(void)
{
	...
	{ 
		elapseTime2 += dt;//実際の経過時間
		elapseTime3 += deltaT;//無次元時間
		calculate(deltaT); 
	...
	{
		Courant = 1.0 * deltaT / rect.delta.x;	
		diff_num = (1.0 / Re) * deltaT / (rect.delta.x * rect.delta.x);//拡散数
	}
	else
	{
		Courant = 1.0 * deltaT / rect.delta.y;	
	...
	if(flagParameter)
	{
		...
		drawParam("deltaT=%1.4f", deltaT, 0.2, 0.9);
...
/*
	//流れ関数境界条件
	for (j = 1; j < NY; j++) {
		//Psi[0][j] = (float)j * rect.delta.y;//Dirichlet
...
void drawRegion()
{
	...
	//左下側
	glVertex2f(rect.left0.x, rect.left0.y); 
	glVertex2f(rect.left0.x + scale.x * (rect.obs_left - rect.obs_thick/2.0), rect.left0.y);
	//障害物左端
	glVertex2f(rect.left0.x + scale.x * (rect.obs_left - rect.obs_thick/2.0), rect.left0.y);
	glVertex2f(rect.left0.x + scale.x * (rect.obs_left - rect.obs_thick/2.0), rect.left0.y + scale.y * rect.obs_width);
	//障害物上部
	glVertex2f(rect.left0.x + scale.x * (rect.obs_left - rect.obs_thick/2.0), rect.left0.y + scale.y * rect.obs_width);
	glVertex2f(rect.left0.x + scale.x * (rect.obs_left + rect.obs_thick/2.0), rect.left0.y + scale.y * rect.obs_width);
	//障害物右端
