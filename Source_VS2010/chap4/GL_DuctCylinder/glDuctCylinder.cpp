...
grid表示で境界上の格子点が赤で表示されるので6角形近似か確認
*/
#include <windows.h>
#include <GL/glut.h>
...
enum Type{INSIDE, INLET, OUTLET, TOP, BOTTOM, OBSTACLE, OBS_LEFT, OBS_TOP, OBS_BOTTOM,
          OBS_RIGHT, OBS_UL, OBS_UR, OBS_LL, OBS_LR};
...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GL_DuctCylinder");
	//ウィンドウのサイズ変更
...
void init(void)
{
	...
	//サイズの確認
	float eps = 0.00001;
	if( fabs((float)(int(rect.obs_left / DX )) * DX - rect.obs_left) > eps) { printf("円柱の中心点が格子点に一致していない \n"); flagResetting = true;}
	if( fabs((float)(int(rect.obs_radius / DX)) * DX - rect.obs_radius) > eps) {printf("円柱の半径が格子点間隔の整数倍に一致していない \n"); flagResetting = true;}
	if( fabs(DX - DY) > eps) {printf("格子間隔が不一致 \n"); flagResetting = true;}
	...
	for(i = 0; i <= NX; i++)
		for(j = 0; j <= NY; j++)
		{
			...
			if( r <= r0 ) type[i][j] = OBSTACLE;//ここでは境界を含む

			if( fabs(y - y0 - r0) < eps ) type[i][j] = OBS_TOP;
			else if( fabs(y - y0 + r0) < eps ) type[i][j] = OBS_BOTTOM;
			else if( fabs(x - x0 + r0) < eps ) type[i][j] = OBS_LEFT;
			else if( fabs(x - x0 - r0) < eps ) type[i][j] = OBS_RIGHT;
			else
			{
				...
				if(i == int(x1/DX + 0.5) && y > y0 ) type[i][j] = OBS_UL;//左上
				if(j == int(y2/DY + 0.5) && x < x0 ) type[i][j] = OBS_UL;//左上
				if(i == int(x1/DX + 0.5) && y < y0 ) type[i][j] = OBS_LL;//左下
				if(j == int(y1/DY + 0.5) && x < x0 ) type[i][j] = OBS_LL;//左下
				if(i == int(x2/DX + 0.5) && y > y0) type[i][j] = OBS_UR;//右上
				if(j == int(y2/DY + 0.5) && x > x0) type[i][j] = OBS_UR;//右上
				if(i == int(x2/DX + 0.5) && y < y0) type[i][j] = OBS_LR;//右下
				if(j == int(y1/DY + 0.5) && x > x0) type[i][j] = OBS_LR;//右下
			}
		}
	...
	for (i = 0; i <= NX; i++)
		for(j = 0; j <= NY; j++)
		{
			...
			//速度
			Vel[i][j].x = 1.0;
			if(type[i][j] == BOTTOM || type[i][j] == TOP || type[i][j] >= OBSTACLE) Vel[i][j].x = 0.0;
			Vel[i][j].y = 0.0;
			gx[i][j] = 0.0;
			gy[i][j] = 0.0;
		}
...
void display(void)
{
	...
	if(flagParameter)
	{
		...
		drawParam("e_time=%3.3f[s]", elapseTime2, -0.1, 0.9);
//		drawParam("deltaT=%6.2f", deltaT, 0.2, 0.9);
		drawParam("n_time=%3.3f", elapseTime3, 0.3, 0.9);
...
void calculate(float deltaT)
{
	...
	//流れ関数境界条件
	for (j = 1; j < NY; j++) {
		//Psi[0][j] = (float)j * rect.delta.y;//Dirichlet(省略可）
		Psi[NX][j] = Psi[NX-1][j];//Neumann
	}
	...
	for (i = 0; i <= NX; i++)
		for (j = 0; j <= NY; j++) 
		{
			...
			else if(type[i][j] == OBS_UL) Omega[i][j] = -2.0 * (Psi[i-1][j+1] - psi_obs) / DD2;
			else if(type[i][j] == OBS_UR) Omega[i][j] = -2.0 * (Psi[i+1][j+1] - psi_obs) / DD2;
			else if(type[i][j] == OBS_LL) Omega[i][j] = -2.0 * (Psi[i-1][j-1] - psi_obs) / DD2;
			else if(type[i][j] == OBS_LR) Omega[i][j] = -2.0 * (Psi[i+1][j-1] - psi_obs) / DD2;
		}
	...
	if(flagParameter)
	{
		...
		drawParam("maxerror=%2.7f", maxError, 0.1, 0.8);
	}
...
void drawCylinder()
{
	...
	pos0 = Vector2D(rect.obs_left, rect.size.y/2.0);//円柱中心
	...
	for(i = 0; i < nn; i++)
	{
		pos1.x = rect.obs_radius * cos((float)i * theta0);
		pos1.y = rect.obs_radius * sin((float)i * theta0);
		pos2.x = rect.obs_radius * cos((float)(i+1) * theta0);
		pos2.y = rect.obs_radius * sin((float)(i+1) * theta0);
		pos1 += pos0;
		pos2 += pos0;
			
		glBegin(GL_TRIANGLES);
		glVertex2f(rect.left0.x + scale.x * pos0.x, rect.left0.y + scale.y * pos0.y);
		glVertex2f(rect.left0.x + scale.x * pos1.x, rect.left0.y + scale.y * pos1.y);
		glVertex2f(rect.left0.x + scale.x * pos2.x, rect.left0.y + scale.y * pos2.y);
		glEnd();
	}
...
void drawGrid()
{
	...
	for(j = 1; j < rect.nMeshY; j++)
	{
		...
		if(type[i][j] == OBS_TOP || type[i][j] == OBS_BOTTOM || type[i][j] == OBS_LEFT || type[i][j] == OBS_RIGHT) glColor3f(1.0, 0.0, 0.0); 
