...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GP_DuctCylinder");
	//ウィンドウのサイズ変更
...
void initData()
{
	...
	//サイズの確認
	float eps = 0.00001;
	if( fabs((float)(int(rect.obs_left / DX + 0.5)) * DX - rect.obs_left) > eps) { printf("円柱の中心点が格子点に一致していない \n"); flagResetting = true;}
	if( fabs((float)(int(rect.obs_radius / DX + 0.5)) * DX - rect.obs_radius) > eps) {printf("円柱の半径が格子点間隔の整数倍に一致していない \n"); flagResetting = true;}
	...
	for(j = 0; j < texHeight; j++)
		for(i = 0; i < texWidth; i++)
		{
			...
			if(g_type[i][j] >= OBSTACLE) psi[3 * k + 0] = rect.size.y / 2.0;//障害物表面
			psi[3 * k + 1] = 1.0;//x方向速度
	...
	rigid[0].vSize = Vector3D(2.0*rect.obs_radius, 2.0*rect.obs_radius, 1.0);
printf("rect.size.x=%f, rect.obs_left=%f, rect.obs_radius=%f \n", rect.size.x, rect.obs_left, rect.obs_radius);
...
//	float pp;
	...
	if(flagStart==true && flagFreeze == false && flagFreeze0 == false) 
	{
	...
	for(i = 1; i < NX; i++)
		for (j = 1; j < NY; j++)
		{
			...
			if(g_type[i][j] >= OBSTACLE) continue;
