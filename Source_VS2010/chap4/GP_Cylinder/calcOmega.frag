...
void main(void)
{
	...
	if(i > 0 && i < NX && j >= 0 && j < NY)
	{
		...
		f += ( ((c30 * x + c21 * y + c20) * x + c11 * y + gx) * x
			       + ((c03 * y + c12 * x + c02) * y + gy) * y ) * ex;

		gx += ((3.0 * c30 * x + 2.0 * (c21 * y + c20)) * x + (c12 * y + c11) * y) * ex;
		gy += ((3.0 * c03 * y + 2.0 * (c12 * x + c02)) * y + (c21 * x + c11) * x) * ex;
		...
		f += deltaT * ex2 * ( (f_im + f_ip - 2.0 * f) / dx2 
