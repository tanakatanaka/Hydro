...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GP_VortexWave2");
	//ウィンドウのサイズ変更
...
{	
	...
	//障害物境界の分割点番号
	float eps = 0.5;
	int NX1 = nDummy + floor((rect.obs_left - rect.obs_thick / 2.0) / DX + eps);//障害物左端位置
	int NX2 = nDummy + floor((rect.obs_left + rect.obs_thick / 2.0) / DX + eps);//障害物右端位置
	int NY2 = nDummy + floor((rect.obs_posY + rect.obs_width/2.0) / DY + eps);//障害物上端位置
	int NY1 = nDummy + floor((rect.obs_posY - rect.obs_width/2.0) / DY + eps);//障害物下端位置
	...
	for(i = 0; i <= NX; i++)
		for(j = 0; j <= NY; j++)
		{
			...
			if(j == 0) g_type[i][j] = BOTTOM;//下境界
			if(j == NY) g_type[i][j] = TOP;//上境界
			if(i == 0) g_type[i][j] = LEFT;//左
			if(i == NX) g_type[i][j] = RIGHT;//右
...
/*
		rigid[3].kind = CUBE;
		rigid[3].color = GRAY;
		rigid[3].vSize = Vector3D(wid, rect.size.y, hig);
		rigid[3].vPos = Vector3D(rect.size.x/2.0 + 0.5*wid , 0.0, 0.0);

		rigid[4].kind = CUBE;
		rigid[4].color = GRAY;
		rigid[4].vSize = Vector3D(wid, rect.size.y, hig);
		rigid[4].vPos = Vector3D(-rect.size.x/2.0 - 0.5*wid , 0.0, 0.0);
*/
	}
	decideRegion();
	setTextureWave();
...
void calcObsPos()
{
	...
	float eps = 0.5;
	NX1 = nDummy + floor((left.x - rect.obs_thick / 2.0) / DX + eps);//障害物左端位置
	NX2 = nDummy + floor((left.x + rect.obs_thick / 2.0) / DX + eps);//障害物右端位置
	NY2 = nDummy + floor((left.y + rect.obs_width/2.0) / DY + eps);//障害物上端位置
	NY1 = nDummy + floor((left.y - rect.obs_width/2.0) / DY + eps);//障害物下端位置
