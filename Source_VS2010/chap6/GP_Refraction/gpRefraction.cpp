...
//シェーダプログラム名
GLuint shader1, shader2, shader3, shader4, shader5, shader6, shader7;
...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GP_Refraction");
	//ウィンドウのサイズ変更
...
void init(void)
{
	...
	glGenTextures(5, texID);//テクスチャオブジェクトの名前付け
...
void makeLowerTank()
{
	...
	rigid[2].vPos = Vector3D(rect.size.x/2.0 + bWidth/2.0, 0.0, bHeight / 2.0);
	rigid[2].vSize = Vector3D(bWidth, rect.size.y, bHeight);
	//左側
	rigid[3].kind = CUBE;
	rigid[3].color = DARK_YELLOW;
	rigid[3].vPos = Vector3D(-rect.size.x/2.0 - bWidth/2.0, 0.0, bHeight / 2.0);
	rigid[3].vSize = Vector3D(bWidth, rect.size.y, bHeight);
	//奥のブロック
	rigid[4].kind = CUBE;
	rigid[4].color = DARK_CYAN;
	rigid[4].vPos = Vector3D(0.0, rect.size.y/2.0 + bWidth/2.0, bHeight / 2.0);
	rigid[4].vSize = Vector3D(rect.size.x + 2.0*bWidth, bWidth, bHeight);
}
void makeUpperTank()
{
	float bHeight = 7.0 - waveHeight;
	float bWidth = 0.2;
	float hh0 = waveHeight + bHeight / 2.0;
	//右側
	rigid[5].kind = CUBE;
	rigid[5].color = DARK_YELLOW;
	rigid[5].vPos = Vector3D(rect.size.x/2.0 + bWidth/2.0, 0.0, hh0);
	rigid[5].vSize = Vector3D(bWidth, rect.size.y, bHeight);
	//左側
	rigid[6].kind = CUBE;
	rigid[6].color = DARK_YELLOW;
	rigid[6].vPos = Vector3D(-rect.size.x/2.0 - bWidth/2.0, 0.0, hh0);
	rigid[6].vSize = Vector3D(bWidth, rect.size.y, bHeight);
	//奥のブロック
	rigid[7].kind = CUBE;
	rigid[7].color = DARK_CYAN;
	rigid[7].vPos = Vector3D(0.0, rect.size.y/2.0 + bWidth/2.0, hh0);
	rigid[7].vSize = Vector3D(rect.size.x + 2.0*bWidth, bWidth, bHeight);
}
...
void makeTextureOfCubeMapping()
{
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		glPushMatrix();
		//視点から見えるものをレンダリング 
		gluLookAt(view.vPos.x, view.vPos.y, view.vPos.z, view.vPos.x+c_target[i].cx, view.vPos.y + c_target[i].cy, view.vPos.z + c_target[i].cz, c_target[i].ux, c_target[i].uy, c_target[i].uz);
