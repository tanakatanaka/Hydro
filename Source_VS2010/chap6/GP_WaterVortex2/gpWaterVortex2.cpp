...
GLuint fbo[3];//frame buffer object
...
int main(int argc, char** argv)
{
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GP_WaterVortex2");
	//ウィンドウのサイズ変更
	...
	initGlsl(&shader_prog4, "simulation.vert", "particle.frag");//粒子用
	initGlsl(&shader_prog5, "renderingP.vert");//粒子用
...
void init(void)
{
	...
	glGenTextures(3, texID);//テクスチャオブジェクトの名前付け
	glGenFramebuffersEXT(3, fbo);//フレームバッファオブジェクトの生成
...
void rendering()
{
	...
	//質点のマテリアル
	static float diffuse[] = { 0.2, 0.5, 0.9, 1.};
...
void renewParticle()
{
	...
	glUniform1i(samplerPosLoc, 2);//GL_TEXTURE2を適用
...
void calcObsPos()
{
	...
	{
		if(rigid[0].vPos.x >=  rect.size.x/2 - rect.obs_left) direction = -1.0;
		if(rigid[0].vPos.x <= -rect.size.x/2 + rect.obs_left) direction =  1.0;
