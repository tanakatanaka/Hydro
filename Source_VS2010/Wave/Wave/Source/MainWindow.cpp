...
int MainWindow::Initialize(int argc, char** argv)
{
	glutInit(&argc, argv);
	//表示モード
	...
	//ウィンドウ作成
	main_window = glutCreateWindow("GP_Caustics2");
	//ウィンドウのサイズ変更
...
void makeLowerTank()
{
	...
	rigid[6].kind = SPHERE;
	rigid[6].color = GREEN;
	rigid[6].vPos = Vector3D(-8.0, 0.0, 1.5);
	rigid[6].vSize = Vector3D(3.0, 3.0, 3.0);
	rigid[6].vEuler = Vector3D(0.0, 0.0, 0.0);
...
void makeUpperTank()
{
	...
	//LIGHT
	rigid[11].kind = BOX;
	rigid[11].color = WHITE;
	rigid[11].vPos = Vector3D(lightPos[0], lightPos[1], lightPos[2] + 1.0);
	rigid[11].vSize = Vector3D(3.0, 3.0, 0.2);
...
void renewWavePos()
{
	...
	glUniform1i(texWLoc, rect.meshX);
	GLint texHLoc = glGetUniformLocation(shader4, "meshY");
	glUniform1i(texHLoc, rect.meshY);
